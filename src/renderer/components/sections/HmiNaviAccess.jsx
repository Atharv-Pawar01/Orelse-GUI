...
export function HmiNaviAccess() {
  const { canViewHmi } = usePrivilege()
  const [message, setMessage] = useState(null)
  const [loading, setLoading] = useState(null)

  async function handleLaunch(method) {
    if (!window.electronAPI?.hminaviLaunch) {
      setMessage({ type: 'error', text: 'HmiNavi launch not available in this environment.' })
      return
    }
    setMessage(null)
    setLoading(method)
    try {
      const result = await window.electronAPI.hminaviLaunch(method)
      setLoading(null)
      if (result.ok) {
        setMessage({ type: 'success', text: 'HmiNavi application starting.' })
      } else {
    ...
    <div>
      ...
      <div style={styles.cards}>
        <div style={styles.card}>
          <h3 style={styles.cardTitle}>HmiNavi application</h3>
          <p style={styles.cardP}>Launch HmiNavi executable on this PC.</p>
          <button
            type="button"
            style={styles.button}
          ...
          >
            {loading === 'app' ? 'Launching…' : 'Launch HmiNavi'}
          </button>
        </div>
      </div>
    </div>
